...
if (isset($_SESSION['usuario_logado'])) {
    ...
    // Troque dashboard.php ou index.php por painel.php
header("Location: painel.php");
exit();
}

// CORREÇÃO DO CAMINHO: Agora o arquivo está na raiz, então usamos 'config/'
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($db === null) {
        die("Erro grave: Não foi possível conectar ao banco de dados. Verifique o arquivo config/database.php");
    }
    ...
    if (!empty($usuarioInput) && !empty($senhaInput)) {
        try {
            ...
            if ($usuario && ($senhaInput === $usuario['senha'])) {
                ...
                header("Location: dashboard.php"); // Verifique se este arquivo existe na raiz
                exit();
            } else {
